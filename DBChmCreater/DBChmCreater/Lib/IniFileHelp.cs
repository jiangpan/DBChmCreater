...
/*
 * Ini文件结构  （不区分大小写的）iniFile.DeleteKey("section1", "key2");
 * [Section]
 * Key=Value
 * 构造方法：若指定的path无ini文件，会创建
 * 读取值的方法：iniFile.GetInt32("SQL", "TYPE", 0);//参数依次是SectionName,Key,DefaultValue
 *                类似的有 GetString,GetDouble
 * 读取还有其他方法： string[] sections = iniFile.GetSectionNames(); //返回所有的SectionName
 *                      iniFile.GetKeyNames(section);//返回Section下所有的KeyName
 *                      iniFile.GetSectionValues(section);//返回Section下所有的Key，Value （Dictionary）
 *                      List<KeyValuePair<string, string>> lst = iniFile.GetSectionValuesAsList(section);//跟上面的一样，不过是一个链表里存储的是Dictionary对象
 * 写入值的方法： iniFile.WriteValue("section1", "键值1", 42);//参数依次是SectionName,Key,Value
 *                  该方法，是一个重载，Value，可以是int，double，string
 *                  写入的时候，如果该Section对应的Key已存在，则会修改它
 * 删除： iniFile.DeleteKey("section1", "key2");//删除Section下的Key
 *         iniFile.DeleteSection("section2");//直接删除Section，会将该Section下的所有键值都删除
 *         
 * 测试方法：
 *  //测试Ini文件
        public  void Initest()
        {
            IniFileHelp iniFile = new IniFileHelp("D:\\TestIniFile.ini");
